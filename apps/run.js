...
export class run extends plugin {
  ...
  async run(e) {
    ...
    if (e.atme && e.msg == "#回避" || (e.msg.includes('回避') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {
    ...
    else if (e.atme && e.msg == "#回来" || (e.msg.includes('回来') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {//如果消息内容是回来指令
    ...
    else if (e.atme && e.msg == "#跑路" || (e.msg.includes('跑路') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {//如果消息内容是跑路指令
      if (e.sender.role == "owner" || e.sender.role == "admin" || e.isMaster) {//如果是群主或管理员
        if (!json[id].run) {//如果不在跑路列表中
          json[id].run = true;//添加到跑路列表
          json = await lin_data.getdata(e.group_id, json, true)
    ...
    else if (e.atme && e.msg == "#别理我" || e.atme && e.msg == "#屏蔽我" || (e.msg.includes('屏蔽我') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {//如果消息内容是跑路指令
    ...
    else if (e.at && (e.msg.includes('#屏蔽此人')) || (e.msg.includes('屏蔽此人') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {//如果消息内容是跑路指令
      if (e.sender.role == "owner" || e.sender.role == "admin" || e.isMaster) {//如果是群主或管理员
    ...
    if (e.atme && e.msg == "#屏蔽列表" || (e.msg.includes('屏蔽列表') && (e.msg.includes(BotName) || e.msg.includes('全部')))) {//如果消息内容是跑路指令
      if (e.sender.role == "owner" || e.sender.role == "admin" || e.isMaster) {//如果是群主或管理员
