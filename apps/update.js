...
export class update extends plugin {
  ...
  async runUpdate(isForce) {
    ...
    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：麟插件`);
    ...
    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`麟插件已经是最新版本\n最后更新时间：${time}`);
    } else {
      await this.reply(`麟插件\n最后更新时间：${time}`);
      this.isUp = true;
      /** 获取麟组件的更新日志 */
  ...
  async getLog(plugin = "") {
    ...
    let end = "";
      ...
      "更多详细信息，请前往gitee查看\nhttps://gitee.com/yeyang52/lin-plugin";

    log = await this.makeForwardMsg(`麟插件更新日志，共${line}条`, log, end);
